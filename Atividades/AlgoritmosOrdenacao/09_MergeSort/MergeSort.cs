...
namespace MergeSort
{
    ...
    {
        // Metodo para realizar a divisão do array
        public static T[] Sort<T>(T[] array) where T : IComparable
        {
            if (array.Length <= 1) // Se o tamanho do array for menor ou igual a 1, nao ha mais o que dividir
                return array;
        ...
        // Metodo para mesclar os arrays
        public static T[] Merge<T>(T[] left, T[] right) where T : IComparable
        {
            T[] result = new T[left.Length + right.Length]; // Array que armazenara o resultado final
            ...
            while (i < left.Length && j < right.Length) {
                if (left[i].CompareTo(right[j]) <= 0) // Se left[i] for menor ou igual a right[j]
                    ...
                    result[k++] = right[j++]; // Caso contrario, copia o right[j] para o result
            }
